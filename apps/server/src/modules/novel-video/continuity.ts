import type { ContinuityIssue } from "@angry-mushroom/shared";
...
/**
 * Validate shot continuity using deterministic rules.
 * MVP checks: missing references, action/emotion mismatches,
 * facing changes, and forbidden camera angles.
 */
export function validateShotContinuity(args: {
  ...
  for (const shot of shots) {
    // Rule 1: Shot must have a valid scene
    ...
    // Rule 2: Shot must have valid characters
    ...
    // Rule 6: Camera angle must not be in forbidden angles for this scene
  ...
  // Check consecutive shots in the same scene for continuity issues
  for (let i = 1; i < shots.length; i++) {
    const prev = shots[i - 1];
    const curr = shots[i];

    // Only check if same scene
    if (prev.locationId !== curr.locationId) continue;

    // Rule 3: Character facing should not change without reason
    ...
    // Rule 4: Action continuity — prev actionEnd should match curr actionStart
    if (
      prev.continuity.actionEnd &&
      curr.continuity.actionStart &&
      prev.continuity.actionEnd !== curr.continuity.actionStart
    ) {
      // Only warn if the descriptions are very different
    ...
    // Rule 5: Emotion continuity — prev emotionEnd should match curr emotionStart
    if (
